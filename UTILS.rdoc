...
avg = 0 
(1..w).each do |i|
avg = avg+ u.runs.where(:datetime => i.week.ago.beginning_of_week..i.week.ago.end_of_week).sum(:distance).to_f
end
meta.push(((avg / w)*1.05).to_f)
end
if meta.min <3
	meta.delete(meta.min)
end
if meta.min.nil? || meta.min <3
	meta = [8]
end
puts %{meta ->#{meta.min}}
...
u = User.find_by(:email=>"alexandresantos@mail.com")
